...
export function baghitground(bag) {
  var bn, b, clbits;
...
export function bagy(bag) {
  return bagdat[bag].y;
}

export function cleanupbags() {
  var bpa;
  Sound.soundfalloff();
  for (bpa = 1; bpa < 8; bpa++) {
...
export async function drawbags() {
  var bag;
  for (bag = 1; bag < 8; bag++) {
    if (Main.getcplayer() == 0)
...
export function getbagdir(bag) {
  if (bagdat[bag].exist)
    return bagdat[bag].dir;
  return -1;
}

export function getgold(bag) {
  var clbits;
...
export function getnmovingbags() {
  var bag, n = 0;
  for (bag = 1; bag < 8; bag++)
...
export function pushbag(bag, dir) {	// r: boolean
...
export function pushbags(dir, bits) {  // r: boolean
  var bag, bit;
  var push = true;
...
export function pushudbags(bits) { // r: boolean
  var bag, b;
  var push = true;
  for (bag = 1, b = 2; bag < 8; bag++, b <<= 1)
    if ((bits & b) != 0)
...
export function removebags(bits) {
  var bag, b;
  for (bag = 1, b = 2; bag < 8; bag++, b <<= 1)
