...
export function checkcoincide(mon, bits) {
  var m, b;
...
export function checkmonscared(h) {
  var m;
  for (m = 0; m < 6; m++)
...
export function fieldclear(dir, x, y) {
  switch (dir) {
    case 0:
      if (x < 14)
...
export function incpenalties(bits) {
  var m, b;
  for (m = 0, b = 256; m < 6; m++, b <<= 1) {
    if ((bits & b) != 0)
      Main.incpenalty();
...
export function killmonsters(bits) {
  var m, b, n = 0;
  for (m = 0, b = 256; m < 6; m++, b <<= 1)
    if ((bits & b) != 0) {
      killmon(m);
...
export function squashmonster(mon, death, bag) {
  mondat[mon].alive = false;
  mondat[mon].death = death;
  mondat[mon].bag = bag;
}

export function squashmonsters(bag, bits) {
  var m, b;
  for (m = 0, b = 256; m < 6; m++, b <<= 1)
    if ((bits & b) != 0)
